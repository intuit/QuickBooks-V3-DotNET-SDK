...
namespace Intuit.Ipp.Core.Rest
{
    ...
    public abstract class RestHandler : IRestHandler
    {
            ...
            : this()
        {
            ...
            RequestCompressor = CoreHelper.GetCompressor(serviceContext, true);
            ResponseCompressor = CoreHelper.GetCompressor(serviceContext, false);
            RequestSerializer = CoreHelper.GetSerializer(serviceContext, true);
            responseSerializer = CoreHelper.GetSerializer(serviceContext, false);
            RequestLogging = CoreHelper.GetRequestLogging(serviceContext);
        ...
        /// </returns>
        public virtual HttpWebRequest PrepareRequest(RequestParameters requestParameters, object requestBody, string oauthRequestUri, bool includeRequestId = true)
        {
            //initialize the Advanced logger
            CoreHelper.AdvancedLogging = CoreHelper.GetAdvancedLogging(serviceContext);
            serviceContext.IppConfiguration.Logger.CustomLogger.Log(TraceLevel.Info, "Called PrepareRequest method");
            ...
            Include = serviceContext.Include;
            MinorVersion = serviceContext.MinorVersion;
            ...
            {
                requestEndpoint = serviceContext.BaseUrl + StripFirstSlash(requestParameters.ResourceUri);
            }
            else
            {
            ...
            {
                requestEndpoint = AppendQueryParameters(requestEndpoint, "include", string.Join(",", Include.ToArray()));
            }

            requestEndpoint = !String.IsNullOrWhiteSpace(MinorVersion) ? AppendQueryParameters(requestEndpoint, "minorversion", MinorVersion) : AppendQueryParameters(requestEndpoint, "minorversion", Properties.Resources.DefaultMinorVersionValue);

            if (!String.IsNullOrWhiteSpace(RequestId))
            {
                if (requestBody != null && requestBody is Data.IntuitBatchRequest)
                {
                    //The maximum request ID + batch ID length is 50 characters.
                    //For backwards compatibility, truncate the request ID if necessary.
                    Data.IntuitBatchRequest batchRequest = (Data.IntuitBatchRequest)requestBody;
                    ...
                    {
                        RequestId = RequestId.Substring(0, (int)Math.Min(RequestId.Length, 50 - maximumBatchIdLength));
                    }
                }
            ...
            if (responseSerializer is JsonObjectSerializer)
            {
            ...
            if (serviceContext.ServiceType == IntuitServicesType.IPS && (oauthRequestUri == null))
            {
            ...
            {
                httpWebRequest.Headers.Add(CoreConstants.CONTENTENCODING, RequestCompressor.DataCompressionFormat.ToString().ToLowerInvariant());
            }

            if (ResponseCompressor != null)
            {
                httpWebRequest.Headers.Add(CoreConstants.ACCEPTENCODING, ResponseCompressor.DataCompressionFormat.ToString().ToLowerInvariant());
            }
                ...
                if (requestParameters.Verb == HttpVerbType.POST)
                {
                    ...
                    if (streamRequestBody == null)
                    {
                        ...
                        if (requestBody != null)
                        {
                            ...
                            RequestLogging.LogPlatformRequests(" RequestUrl: " + requestEndpoint + ", Request Payload:" + requestXML.ToString(), true);
                            //Log to Serilog
                    ...
                    serviceContext.IppConfiguration.Logger.CustomLogger.Log(TraceLevel.Info, (int)traceSwitch.Level > (int)TraceLevel.Info ? "Adding the payload to request.\n Start dump: \n" + requestXML.ToString() : "Adding the payload to request.");
            ...
            serviceContext.IppConfiguration.Security.Authorize(httpWebRequest, requestBody == null ? null : requestBody.ToString());
        ...
        {
            StringBuilder requestEndpointBuilder = new StringBuilder(requestEndpoint);
        ...
        /// <param name="request">The request.</param>
        /// <returns>Response from REST service.</returns>
        public abstract byte[] GetResponseStream(HttpWebRequest request);
    }
}
