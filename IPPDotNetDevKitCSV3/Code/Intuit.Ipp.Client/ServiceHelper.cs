...
internal sealed class ServicesHelper
{
    ...
    internal static void ValidateEntity<T>(T entity, ServiceContext serviceContext) where T : IEntity
    {
        if (!IsTypeNull(entity))
        {
            ...
            serviceContext.IppConfiguration.Logger.CustomLogger.Log(Intuit.Ipp.Diagnostics.TraceLevel.Error, string.Format(CultureInfo.InvariantCulture, Resources.ExceptionGeneratedMessage, exception.ToString()));
