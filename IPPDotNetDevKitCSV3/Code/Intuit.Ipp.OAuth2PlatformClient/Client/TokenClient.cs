    ...
    public class TokenClient : IDisposable
    {
        ...
        public virtual async Task<TokenResponse> RequestAsync(IDictionary<string, string> form, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (OAuth2Client.AdvancedLoggerEnabled != false)
            {
                ...
                OAuth2Client.OAuth2TraceLogger.Log(OAuth2TraceLevel.Info, "Authorization Header: " + request.Headers.Authorization.ToString());
                OAuth2Client.OAuth2TraceLogger.Log(OAuth2TraceLevel.Info, "ContentType header: " + request.Content.Headers.ContentType.ToString());
                OAuth2Client.OAuth2TraceLogger.Log(OAuth2TraceLevel.Info, "Accept header: " + "application/json");
                OAuth2Client.OAuth2TraceLogger.Log(OAuth2TraceLevel.Info, "Request Body: " + await request.Content.ReadAsStringAsync().ConfigureAwait(false));
            }
            try
            {
                    ...
                    else
                    {
                        if (OAuth2Client.AdvancedLoggerEnabled != false)
                        {
                            if (OAuth2Client.ShowInfoLogs == true)//log just intuit_tid for info logging mode
                                ...
                                OAuth2Client.OAuth2TraceLogger.Log(OAuth2TraceLevel.Info, "Response: Status Code- " + response.StatusCode + ", Error Details- " + response.ReasonPhrase);
